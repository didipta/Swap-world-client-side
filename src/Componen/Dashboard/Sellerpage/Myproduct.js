    ...
    <tbody>
      {
        allproduct.map(product=>
            ...
            <td>
              {
                product?.status==="Sold out"||product?.status==="Booking"?<>{product?.status==="Booking"?
                <label className="btn btn-xs bg-red-400 border-none text-white">Booking cancel</label>:
                <p className="text-xs font-medium text-orange-500">{product?.status}</p>}</>:
                product?.status==="advertised"?
                <p className="text-xs font-medium text-teal-500">{product?.status}</p>:
                <label htmlFor="admin-modal" className="btn btn-xs" onClick={()=>{SetUserid(product._id)}}>advertise</label>
              }
            </td>
          </tr>  
            )
